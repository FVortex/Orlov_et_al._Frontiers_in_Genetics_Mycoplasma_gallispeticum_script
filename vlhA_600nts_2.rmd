...
which(str_detect(vlha_600, pattern = '[^(gaa)](gaa){12}(gtt)'))
...
gaas <- list()
for (n in 5:30){
  gaas[[n]] <- which(str_detect(vlha_600, pattern = paste0('[^(gaa)](gaa){', n, '}(?!(gaa))')))
  gtt_after_gaas[[n]] <- which(str_detect(vlha_600, pattern = paste0('[^(gaa)](gaa){', n, '}gtt')))
  ctt_after_gaas[[n]] <- which(str_detect(vlha_600, pattern = paste0('[^(gaa)](gaa){', n, '}ctt')))
}


df1 <- cbind(sapply(gaas,length), sapply(gtt_after_gaas,length), sapply(ctt_after_gaas,length),
             sapply(gtt_after_gaas,length)/sapply(gaas,length),
             sapply(ctt_after_gaas,length)/sapply(gaas,length),
             sapply(gtt_after_gaas,length)/sapply(gaas,length)+             sapply(ctt_after_gaas,length)/sapply(gaas,length))

colnames(df1) <- c('gaas', 'gtt_after_gaas', 'ctt_after_gaas', 'gaagtt-s / gaaxxx-s', 'gaactt-s / gaaxxx-s', 'gaagtt-s + gaactts/ gaaxxx-s')
View(df1)
...
```{r}
length(which(str_detect(vlha_600_chars, pattern = '[^(gaa)](gaa){4,20}(?!(gaa))')))
#20 is max
which.4_20 <- which(str_detect(vlha_600, pattern = '[^(gaa)](gaa){4,20}(?!(gaa))'))

str_located <- c()
substrings_to_logo_lefts <- c()
substrings_to_logo_rights <- c()
for (i in vlha_600[which.4_20]){
...
```
```{r gtt vs ctt}
which_gtt <-  which(str_detect(vlha_600, pattern = paste0('[^(gaa)](gaa){4,20}gtt')))
which_ctt <- which(str_detect(vlha_600, pattern = paste0('[^(gaa)](gaa){4,20}ctt')))

ggseqlogo(sapply(substrings_to_logo_lefts, FUN = function(x) {paste0(toupper(x), collapse = '')}),  ncol = 1)+ scale_y_continuous(name="Информационнное содержание (бит)")+scale_x_discrete(name ="Последовательность относительно приблизительного положения ТСТ (п.н.)", limits=seq_along(substrings_to_logo[1]), labels =new_labels)+coord_fixed(ratio = 7)
```

```{r New data }
library(seqinr)
vlhA_new <- read.fasta('gaa_for_physics(1).fasta', as.string = T, seqonly = F)
...
table(strands_vlhA)
nonvlhA_new <- read.fasta('S6_promoters_all_genes.fasta', as.string = T, seqonly = F)
...
Ldyn <- lapply(vlhA_new, FUN = function(x) {dynchars(seq = (strsplit(x, split = '')[[1]]), average_interval_size = dyn_int, tss = 400-40, boundaries = c(100, 100), strand = 'forward')}) # transcript is to the left of TSS! until reversed
...
mat_ep <- sapply(Lep, function(x) {x})
means_to_mat <- c(apply((mat_ep),  MARGIN = 1, FUN = median))
`Sequence (angstrom)` <- (1:nrow(mat_ep))
